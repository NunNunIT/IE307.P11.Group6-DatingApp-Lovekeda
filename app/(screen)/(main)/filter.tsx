...
export default function FilterScreen() {
  const [rangeAge, setRangeAge] = useState<[number, number]>([18, 100]);
  ...
  return (
    <ScrollView className="flex-1">
      <View className="h-full p-4 gap-4 flex flex-col mb-16">
        ...
        <View className="relative flex flex-col rounded-lg gap-6 bg-white dark:bg-zinc-900 shadow p-4">
          ...
          <View className="flex flex-row gap-2">
            {["female", "male", "all"].map((gender) => (
              <Pressable
                key={gender}
                className={`h-24 w-full flex-1 flex flex-col justify-center items-center rounded-lg border-2 ${genderValue === gender
                  ? "border-pri-color bg-white dark:bg-zinc-950"
                  : "border-zinc-200 dark:border-zinc-800"
                  }`}
        ...
        {/* Nút Submit */}
        <Button onPress={submitHandler} className="mt-4" variant="red">
          <Text>Xác nhận</Text>
        </Button>
      </View>
    </ScrollView>
